...
class getGwInfo(TestCase):
    ...
    def test_a_get_gwinfo(self):
        '''获取网关信息'''
        self.udp(self.getParam().get_gwinfo)
        self.res = self.udp_send(self.in_param)
    ...
    def test_b_get_gwinfo(self):
        '''cmd错误'''
        self.udp(self.getParam().get_gwinfo_a)

    def test_c_get_gwinfo(self):
        '''参数错误'''
        self.udp(self.getParam().get_gwinfo_b,switch=False)
        self.assertEqual(self.res['status'],200)
